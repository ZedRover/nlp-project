...
class BERTModel(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_nb):
        ...
        val_acc = accuracy_score(
            y_hat.cpu().detach().numpy(), label.cpu().detach().numpy())
    ...
    def test_epoch_end(self, outputs):
        ...
        self.log_dict({'avg_test_loss': avg_loss,
...
def main():
    ...
    df['reviews'] = df.apply(lambda i: i.reviews.replace(
        '女权', '').replace('女性', ''), axis=1)
    df_insmp, df_test = train_test_split(df, test_size=0.2, shuffle=True)
    df_train, df_valid = train_test_split(
        df_insmp, test_size=0.1, shuffle=False)
