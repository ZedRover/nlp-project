...
class TextCNN(LightningModule):
    ...
    def __init__(self, vocab_size, embedding_dim, num_labels, filter_num=100, kernel_list=(3, 4, 5), dropout=0.5,lr=1e-3):
        ...
        self.lr = lr
        self.fc = nn.Linear(filter_num * len(kernel_list), num_labels)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log_dict({
            'val_acc':accuracy_score(target.cpu().detach().numpy(), y_hat.cpu().detach().numpy()),
            'val_loss':loss
        })
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log('test_acc',accuracy_score(output.cpu().detach().numpy(), target.cpu().detach().numpy()))
...
if __name__=='__main__':
    ...
    WEMD_LEN = 300
    wandb.login()
    wandb_logger = WandbLogger(project='cnn-clf',save_dir='./wnb_logs/',offline=False)
    ...
    cnn = TextCNN(vocab_size=SEQ_LEN, embedding_dim=WEMD_LEN, num_labels=2,lr=1e-3)
